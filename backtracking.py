import pygame
import os
from backtracking_algorithm import backtracking_algorithm
...
def backtracking():
    ...
    def draw_board():
        for i in range(32):
            ...
            text1 = font.render('Next Move' , True , color)
            if 480 <= mouse[0] <= 630 and 680 <= mouse[1] <= 720:
                pygame.draw.rect(screen,color_light,[480,680,150,40])
            else:
                pygame.draw.rect(screen,color_dark,[480,680,150,40])
            screen.blit(text1 , (490,690))
    ...
    def draw_game_over():
        ...
        screen.blit(font.render( 'No More Valid Moves!', True, 'white'), (10, 660))
        screen.blit(font.render('Press ENTER To Restart!', True, 'white'), (10, 690))
        screen.blit(font.render('Press SPACE To Go Main Menu!', True, 'white'), (10, 720))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and not game_over:
                ...
                if click_coords in board:
                    ...
                    solution = backtracking_algorithm(x_coords, y_coords)
                    location = click_coords
                    board = []
                    pygame.mixer.Sound.play(move)
                elif 480 <= event.pos[0] <= 630 and 680 <= event.pos[1] <= 720:
                    next = solution.pop()
                    location = next
                    history.append(next)
                    score += 1
